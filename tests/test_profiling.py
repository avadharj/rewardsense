...
class TestCreditCardStatistics:
    ...
    def test_calculate_statistics(self):
        ...
        df = pd.DataFrame([
            {'card_id': 'c1', 'issuer': 'CHASE', 'network': 'VISA', 'annual_fee': 550, 
             'currency': 'POINTS', 'is_business': False, 'discontinued': False},
            {'card_id': 'c2', 'issuer': 'AMEX', 'network': 'AMEX', 'annual_fee': 0,
             'currency': 'MILES', 'is_business': True, 'discontinued': False},
        ])
...
class TestTransactionStatistics:
    ...
    def test_calculate_statistics(self):
        """Test statistics calculation."""
        df = pd.DataFrame([
            {'transaction_id': 'txn_001', 'user_id': 'user_0001', 'date': '2025-08-01',
             'category': 'dining', 'merchant': 'Starbucks', 'amount': 10.0},
            {'transaction_id': 'txn_002', 'user_id': 'user_0001', 'date': '2025-08-02',
             'category': 'travel', 'merchant': 'United', 'amount': 200.0},
        ])
...
class TestUserProfileStatistics:
    ...
    def test_calculate_statistics(self):
        """Test statistics calculation."""
        df = pd.DataFrame([
            {'user_id': 'user_0001', 'archetype': 'high_roller', 'monthly_budget': 10000,
             'age_group': '51-65', 'location_type': 'urban', 'redemption_preference': 'travel_portal'},
            {'user_id': 'user_0002', 'archetype': 'budget_conscious', 'monthly_budget': 2000,
             'age_group': '26-35', 'location_type': 'rural', 'redemption_preference': 'cash_back'},
        ])
...
class TestDataProfiler:
    ...
    def test_profile_transactions(self, tmp_path):
        ...
        df = pd.DataFrame([
            {'transaction_id': 'txn_001', 'user_id': 'user_0001', 'date': '2025-08-01',
             'category': 'dining', 'merchant': 'Test', 'mcc_code': 5812, 'amount': 10.0, 'card_used': 'Card'},
        ])
